...
分隔符：     (, ), ;, ,
注释：       // 单行注释
AST节点：   Number, Variable, BinaryOp, UnaryOp
语法检查：   完整表达式解析，多余Token检测
```

**下一阶段计划**：
- 语句解析（if, while, for等控制结构）
- 函数声明和调用解析
- 作用域和符号表
- 语义分析（类型检查）
...
│   ├── test_simple.cpp # 基础功能测试
│   ├── test_all_operators.cpp # 运算符测试
│   └── test_parser.cpp # 语法分析器测试
├── examples/           # 示例代码
│   ├── lexer_demo.cpp  # 词法分析器演示
│   └── parser_demo.cpp # 语法分析器演示
├── docs/               # 设计文档
├── build/              # 构建输出目录
├── .gitignore          # Git忽略文件
└── Makefile            # 通用构建系统
